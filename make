...
class Simple_Cookie_Control_Generator
{
    public $dir =__DIR__;
        ...
        */
        ...
        while (!empty($dirStack)) {
            ...
            while (false !== ($node = $dir->read())) {
                if ( ($node == '.eslintignore') || ($node == '.eslintrc.json') || ($node == 'package-lock.json') || ($node == 'LICENSE.txt') || ($node == '.vscode') || ($node == 'tests') || ($node == 'node_modules') || ($node == 'vendor') || ($node == 'tests') ||($node == 'phpunit.xml') ||($node == 'gruntfile.js') ||($node == 'gulpfile.js') ||($node == 'package.json') || ($node == '..') || ($node == '.') || ($node=='.git')  || ($node=='composer.json') || ($node=='composer.lock') || ($node=='.gitignore') || ($node =='simple-cookie-control-generator') || ($node=='readme.md') || ($node=='bitbucket-pipelines.yml') || ($node=='.gitmodules') || ($node=='.editorconfig') || ($node=='.jshintrc') || ($node=='make') ) {
                    continue;
                }
 ...
 * @subpackage Simple_Cookie_Control/includes
 */
 ...
 */
class Simple_Cookie_Control_$data[2] {
    ...
    public static function name_of_the_function() {
        ...
        file_put_contents(__DIR__."/$data[3]/class-simple-cookie-control-$data[2].php", $input);
        exit("File class-simple-cookie-control-$data[2].php created into $data[3] folder");
    }

}

$plugin_name_generator = new Simple_Cookie_Control_Generator();
